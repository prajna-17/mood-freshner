			...
			if (res.ok) {
				...
				await fetch(`${API_BASE}/notifications`, {
					...
					body: JSON.stringify({
						...
						message: `${amount} coins have been credited to your account.`,
					}),
				});

				showToast(
					`✓ Approved — ${amount} coins credited to ${userName}`,
				);
				fetchUsers();
			} else {
			...
			if (res.ok) {
				...
				await fetch(`${API_BASE}/notifications`, {
					...
					body: JSON.stringify({
						...
						message: "Your coin request has been rejected by the admin.",
					}),
				});
      ...
      `}</style>
			...
			>
				<div style={{ maxWidth: 900, margin: "0 auto" }}>
					...
					>
						Approve coin purchases · manually credit coins ·
						view balances
					</p>
					...
					<div>
						{allPending.length === 0 ? (
							<div className="card empty-state fade-up">
								...
								>
									All coin purchase requests have
									been resolved
								</p>
							</div>
						) : (
							...
							>
								...
								>
									...
									>
										These users have paid & are
										waiting for coins to be
										credited
									</p>
								</div>
