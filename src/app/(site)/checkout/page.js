...
function CheckoutContent() {
	...
	const coinsToApply = useCoins
		? Math.min(availableCoins, maxApplicable)
		: 0;
	...
	const handlePlaceOrder = async () => {
		...
		if (invalidItems.length > 0) {
			setOrderError(
				`"${invalidItems[0].title}" is not available in your area`,
			);
			return;
		}
		...
		try {
			...
			if (msg.includes("shippingAddress")) {
				msg =
					"Please fill all required address details (Name, Phone, Address)";
			}
			...
			<div className="flex-1 overflow-y-auto pb-36 px-4 pt-5 space-y-4">
				...
				<div className="pay-block bg-white rounded-3xl shadow-sm border border-sky-100 p-5">
					...
					<div className="space-y-3">
						...
						<div
							...
							onClick={() =>
								!isFullyPaidByCoins && setPayment("cod")
							}
						>
							<div
				...
				<div className="px-1 pt-4 pb-6">
					<button
						...
						disabled={
							!address || placing || cartItems.length === 0
						}
